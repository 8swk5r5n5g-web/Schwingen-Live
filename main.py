...
def get_document_title(href, link_text):
    ...
    if "schlussrangliste" in combined or "schlussrang" in combined or "-rl.pdf" in combined or "_rl.pdf" in combined:
        return "Schlussrangliste"

    if "statistik" in combined or "-st.pdf" in combined or "_st.pdf" in combined:
...
def process_pdf(pdf_url, document_title, fest, infos, state):
    ...
    storage_key = f"{fest['anlass_id']}::{filename}"

    if storage_key in state["known_pdfs"]:
        print(f"Bereits bekannt, wird nie wieder gesendet: {filename}")
        return
...
def main():
    ...
    print("Starte Bot: Aktiv-Feste scannen, Statistik und Schlussrangliste prüfen.")
