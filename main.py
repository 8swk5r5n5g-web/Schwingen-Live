...
def build_scraperapi_url(target_url: str, render: bool = True, premium: bool = False) -> str:
    params = {
...
def get_html(url: str) -> str:
    ...
    scraper_url = build_scraperapi_url(url, render=True, premium=False)
...
def is_allowed_pdf(href: str, link_text: str) -> bool:
    ...
    return any(keyword in combined for keyword in ALLOWED_PDF_KEYWORDS)
...
def send_weekend_agenda_if_needed(state: dict) -> None:
    ...
    lines = [
        "📅 Schwingfeste dieses Wochenende",
        "",
    ]
...
def process_detail_page(detail_url: str, state: dict) -> None:
    ...
    for link in soup.find_all("a", href=True):
        ...
        if not is_allowed_pdf(href, link_text):
            print(f"PDF ignoriert: {link_text} / {href}")
            continue

        pdf_url = normalise_url(href)

        if pdf_url in state["sent_pdfs"]:
            print(f"PDF bereits gesendet: {pdf_url}")
            continue
...
def check_ranglisten(state: dict) -> None:
    ...
    for detail_url in detail_links:
        try:
            print(f"Pruefe Detailseite: {detail_url}")
